...
Executed/reviewed: 2026-08-28. `PASS-A` means an automated test passed; `PASS-H` means the local HTTP
...
| Testnet rejection or timeout           | Structured failure, same cloid can reconcile, never fall back to paper                   | PASS-A + PASS-MANUAL                  | Unit tests cover venue rejection, abort timeout, filled-cloid reconciliation, and no duplicate submission. A funded testnet account subsequently produced a real BTC opening fill visible in both pocketpilot and Hyperliquid; no timeout was forced live.      |
| Testnet partial close                  | Never show `CLOSED` for a partial fill; persist exact remainder and allow explicit retry | PASS-A / PENDING-LIVE-RETEST          | A real testnet close filled 0.00101 of 0.00118 BTC and exposed the prior false-close bug. The quantity-aware adapter/unit test and PostgreSQL integration test now pass; the user manually flattened the old remainder, so no automated live retest was placed. |
